...
static void find_as(u8* argv0) {
  ...
  if (slash) {
    ...
    *slash = '/';
    ...
    tmp = alloc_printf("%s/granota-as", dir);
...
static void edit_params(u32 argc, char** argv) {
  ...
  if (!strncmp(name, "granota-clang", 9)) {

    clang_mode = 1;

    setenv(CLANG_ENV_VAR, "1", 1);

    if (!strcmp(name, "granota-clang++")) {
    ...
    if (!strcmp(name, "granota-g++")) cc_params[0] = getenv("AFL_CXX");
    else if (!strcmp(name, "granota-gcj")) cc_params[0] = getenv("AFL_GCJ");
    ...
    if (!cc_params[0]) {
      ...
      SAYF("\n" cLRD "[-] " cRST
           ...
           "    'granota-clang' utility instead of 'granota-gcc'. If you really have GCC installed,\n"
    ...
    if (!strcmp(name, "granota-g++")) {
      u8* alt_cxx = getenv("AFL_CXX");
      cc_params[0] = alt_cxx ? alt_cxx : (u8*)"g++";
    } else if (!strcmp(name, "granota-gcj")) {
...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    ...
    SAYF("\n"
         ...
         "  CC=/usr/local/bin/granota-afl-gcc ./configure\n"
         "  CXX=/usr/local/bin/granota-afl-g++ ./configure\n\n"
